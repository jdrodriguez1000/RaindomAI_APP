...
| [F-037](#f-037---la-comprobacion-0-del-gate-1-compara-fechas-de-autor-que-si-se-pueden-falsificar-y-tres-archivos-afirman-lo-contrario) | La Comprobacion 0 del Gate 1 compara fechas de autor, que si se pueden falsificar, y tres archivos afirman lo contrario | R-015 | Media | Implementado |
...
| [F-040](#f-040---las-plantillas-nuevas-usan-los-codigos-ft--y-sc--que-projectmd-declara-propuesta-no-declarada) | Las plantillas nuevas usan los codigos `FT-` y `SC-`, que `project.md` declara «propuesta, no declarada» | R-017 | Media | Implementado |
| [F-041](#f-041---t-057-afirma-que-las-plantillas-no-llevan-codigos-instanciados-mas-alla-del-primero-y-el-commit-lo-desmiente) | `T-057` afirma que las plantillas no llevan codigos instanciados «mas alla del primero», y el commit lo desmiente | R-017 | Media | Implementado |
...
| [F-087](#f-087---la-seccion-4-del-informe-publica-trece-supuestos-abiertos-donde-su-propia-orden-devuelve-quince-y-su-propia-enumeracion-suma-quince) | La seccion 4 del informe publica «trece» supuestos abiertos donde su propia orden devuelve quince, y su propia enumeracion suma quince | R-030 | Media | Implementado |
...
| Gravedad | Media |
| Estado | Implementado |
| Registrado en | T-055, T-060 |
| Cerrado en | `9a52cfa` (`R-018`) |
...
- **Que se hizo con lo que `R-016` dejo abierto (2026-09-03, sesion `S-018`):** `manager` corrige el
  tercer sitio. La bitacora de `S-015` en `_persistence/progress.md` recibe una **nota fechada** que
  dice que la afirmacion era falsa tal como estaba implementada y que `D-071` ya cambio el mecanismo
  al orden del grafo; el parrafo original **no se reescribe**, por el mismo criterio con que se
  trato `D-069` en `T-055`. Registrado en `T-060`. **El hallazgo sigue `Aceptado — pendiente`**: lo
  cierra la auditoria siguiente si verifica la correccion sobre el commit de esta sesion, no
  `manager`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. El tercer sitio esta corregido: la bitacora de `S-015` en `_persistence/progress.md` lleva la nota fechada que cita `F-037` y `D-071`, y su bloque reproduce (`git show 6b42d0f:.claude/skills/protocol-gate1/SKILL.md | grep -c \"merge-base --is-ancestor\"` devuelve 3). Los otros dos sitios ya estaban corregidos en `bd8a9ff`. Detalle en `_audit/R-018.md`, secciones 1e y 3.
...
- **Que se hizo:** `manager` lo acepta (2026-09-03, sesion `S-018`). El hallazgo es correcto y
  verificado contra `HEAD` (`6b42d0f`): los quince codigos siguen en las cuatro plantillas y
  `project.md` seguia llamandolos «propuesta, no declarada». Se toma la **primera** de las dos
  correcciones —declararlos— porque es el precedente exacto de `D-034` (`N-`) y `D-038` (`I-`), y
  porque la segunda haria ilegible la plantilla de trazabilidad, que existe para enseñar la cadena
  `N-001 → FT-001 → SC-001`. Se declaran **solo `FT-` y `SC-`**: `VS-`, `TC-` y `ADR-` no aparecen
  instanciados en ningun archivo fuera de `_methodology/` y su declaracion es del Paso 3 de la etapa.
  Decision, alternativas descartadas y la objecion resuelta —que `020_baseline` no esta adoptada— en
  `D-075`. Registrado en `T-061`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. `git show 9a52cfa:project.md | grep -nE '^\| `(FT|SC)-XXX`'` devuelve las lineas 232 y 233 con las dos filas nuevas, y el aviso que las acompaña separa declarar de adoptar, citando `D-075` y este hallazgo.
...
- **Que se hizo:** `manager` lo acepta (2026-09-03, sesion `S-018`). El hallazgo es correcto: el
  patron `\b(N|T|D|A|C|I|F|L|S|R|DT)-[0-9]{3}\b` no casa con `FT-001` porque el limite de palabra
  inicial no aplica delante de la `F` de `FT`, y con el patron ampliado aparecen cinco codigos que no
  son «el primero». Se toman las dos correcciones propuestas: **notas fechadas** junto al bloque de
  `D-073` y junto al parrafo de `T-057` —sin reescribir ninguno de los dos originales— con el patron
  ampliado y su salida real, y la declaracion de que este control se corre en adelante con los
  prefijos de dos letras incluidos. Registrado en `T-062`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. Las dos notas fechadas existen —`grep -c 'Nota del 2026-09-03 (T-062, hallazgo F-041)'` devuelve 1 en `decisions.md` y 2 en `tasks.md`—, publican el patron ampliado y su salida real, y ninguno de los dos bloques originales se reescribio. El punto ciego que le queda al patron sucesor se abre aparte como `F-047`: no reabre este.
...
- **Que se hizo:** `manager` lo acepta (2026-09-03, sesion `S-018`), y acepta tambien la
  recomendacion de fondo del auditor. Los dos defectos son ciertos contra `HEAD` (`6b42d0f`): no
  habia ninguna `T-XXX` ni `DT-XXX` que sostuviera el pendiente, y `L-024` delegaba la decision en
  `report_auditor`, que `project.md` define como quien «no construye, no corrige y no decide». Se
  abre **`DT-003`** —`Propuesta (pendiente del usuario)`, porque pagarla implica escribir sobre
  entradas ya auditadas y ese eje es del usuario— y la frase de `L-024` se sustituye por una nota
  fechada que nombra la deuda. **Las cinco lineas no se reescriben**, como el propio hallazgo
  recomienda: la forma de pagar la deuda es anotar cada una con su forma reejecutable. Registrado en
  `T-063`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. `DT-003` existe con su fila en el indice, estado `No implementada` y confirmacion `Propuesta (pendiente del usuario)`; `grep -c 'lo decide una auditoria' _persistence/lessons.md` devuelve 0, y `L-024` cita `DT-003`.
...
- **Que se hizo:** `manager` lo acepta (2026-09-03, sesion `S-018`). El hallazgo es correcto y la
  correccion es la que propone. Se corrige **ademas una segunda frase que el hallazgo no señalaba** y
  que tiene el mismo defecto y la misma causa —«las cinco apariciones anteriores», que son cinco
  lineas—: corregir una y dejar la otra habria dejado la entrada diciendo las dos cosas. El recuento
  por apariciones se conserva donde ya estaba bien, con su nombre. Registrado en `T-064`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. `L-024` distingue ahora las dos cifras en las dos frases que tenian el defecto (lineas 862-864 y 881), y `grep -cE '\*\*seis\*\* apariciones|\*\*cinco apariciones' _persistence/lessons.md` devuelve 0.
...
- **Que se hizo:** `manager` lo acepta (2026-09-03, sesion `S-018`). El hallazgo es correcto. **El
  informe `_audit/S-017.md` no se reescribe** —es el artefacto de una sesion ya cerrada y auditada, y
  reescribirlo dejaria la auditoria describiendo un estado que ya cambio—; lo que se corrige es la
  causa: la plantilla de la seccion `## 1. Que se hizo` de `protocol-close` pasa a exigir que se
  nombren tambien las **entradas ya existentes** que el commit edita, con su codigo, y dice de que
  orden sale esa lista. Va como hueco en la plantilla y no solo como aviso en un bloque explicativo,
  por la razon que el propio skill ya escribio a proposito de `S-007`: un aviso se lee una vez, un
  hueco se ve cada vez. Registrado en `T-065`.
- **Verificacion de `R-018` (2026-09-03, commit `9a52cfa`):** **cerrado**. La plantilla del Paso 6b de `protocol-close` incorpora la seccion «En un archivo de registro, lo que **nace** no es todo lo que cambia» y dos huecos nuevos en la estructura del informe (`grep -c 'no es todo lo que cambia'` devuelve 1). ⚠️ El hallazgo nuevo `F-046` es un fallo de **aplicacion** de esa regla en el mismo commit, no un defecto de la regla: por eso se abre aparte y este se cierra.
...
~~~
  ...
  La lista que el propio informe imprime debajo tiene 26 lineas, no 28. Ademas la numeracion se
  corrio en una posicion: el informe marca «(=15, =16)» sobre la primera orden, cuando la posicion 15
  de su lista es `grep -c 'Nota del 2026-09-03 (T-060, hallazgo F-037)' _persistence/progress.md`,
  que no es duplicada de nada. Esa orden queda **sin salida publicada** —la lista salta de `--14--` a
  `--19--`— mientras el informe afirma que ninguna de las 28 fallo al reejecutarse.
- **Por que importa:** el Paso 2d existe para que el recuento sea contrastable, y es la pieza que
  `F-039` (`R-016`) y `T-059` (`S-017`) acaban de reparar. La cifra publicada es el unico numero que
  un lector puede comparar sin rehacer el barrido; si es falsa, el control no controla nada. Y una
  orden marcada como duplicada de otra que no lo es desaparece de la verificacion sin que se note.
...
~~~
  ...
  El hunk `+709,10` cae entre `L-019` (657) y `L-020` (721), y el texto cita `F-035` y `F-039`, que
  son los dos casos previos de `L-019`. El parentesis «la entrada de `D-069`» tampoco corresponde a
  ninguna de las dos.
- **Por que importa:** mismo defecto que `F-028` y `F-044`, y en el commit que endurece la plantilla
  justo para eso — el ejemplo que ese endurecimiento escribe en `protocol-close` es literalmente
  «`L-004` y `L-019` (nota de reincidencia)». Una atribucion equivocada es peor que la omision que
  corrige: quien busque la reincidencia abrira `L-020`, no encontrara nada y concluira que el informe
  exagera.
...
- **Que se observo:** la nota fechada de `D-073` cierra con «De aqui en adelante este control se
  ...
  `project.md` si declara: `H-`, el hito.
...
~~~
  ...
  Union de las dos listas (`N T D A C I F L S R DT` + `FT SC VS TC ADR`): falta `H`. Ademas el
  cuantificador `[0-9]{3}` no casaria con la forma declarada para ese codigo (`H-nn`, dos digitos),
  asi que el punto ciego es doble.
- **Por que importa:** `F-041` se abrio porque el ambito del patron era mas estrecho que la frase que
  respaldaba. El patron sucesor se declara vigente sin contrastar su cobertura contra la tabla
  «Codigos», que es el unico sitio donde esa lista se puede cerrar. El dia que una plantilla escriba
  `H-02`, el control devolvera cero y la frase volvera a decir mas de lo que prueba, esta vez sin
  nota al lado.
...
| Cerrado en | `30ce070` (verificado en `R-031`) |
...
- **Que se observo:** la lista del Paso 2d en `_audit/S-031.md` tiene 21 lineas, de las que 19 llevan
  ...
  escribe dos cifras que no salen de ninguna orden: la seccion 7 (linea 280) dice «las 20 lineas 1 a
  20 llevan `<hash>` literal» —falso por una: la linea 15 usa el hash historico `9b3f9ee`—, y la NOTA
  DE CIERRE (linea 305) dice «Las 14 restantes de las 21 lineas» —falso por doce: `21 - 19 = 2`—,
  contradiciendose con la frase siguiente de la misma oracion, que si reparte bien (`19 + 1 + 1`).
  Evidencia completa en `_audit/R-031.md`, seccion 2.
- **Por que importa:** la afirmacion de fondo se sostiene (las 19 estan ancladas y reproducen), pero
  es reincidencia del defecto de `F-087` y `F-088`, en el mismo commit que estrena `D-131`, la
  decision que lo prohibe. Una regla que se incumple en el commit que la escribe no se puede dar por
  vigente sin comprobarlo.
- **Que lo corregiria:** derivar las dos cifras con la orden sobre el bloque ya publicado y pegar su
  salida, en vez de teclearlas.

---

### F-092 - La seccion 4 del informe enumera once codigos de supuesto y afirma que «completan los catorce»
| Campo | Valor |
|---|---|
| Auditoria | R-031 |
| Fecha | 2026-09-10 |
...
- **Que se observo:** la seccion 4 de `_audit/S-031.md` publica bien la salida cruda de su orden
  ...
  `A-004` y `A-005` … completan los catorce». Son `4 + 5 + 2 = 11`; faltan `A-001`, `A-002` y
  `A-003`. La lista real, derivada, es `A-001 A-002 A-003 A-004 A-005 A-006 A-007 A-008 A-009 A-011
  A-012 A-013 A-014 A-015`. Evidencia completa en `_audit/R-031.md`, seccion 2.
- **Por que importa:** el parrafo dice literalmente que no transcribe la lista «para no volver a
  cometer el error que `F-087` acaba de corregir», y transcribe una parcial declarandola completa.
  `A-003` —el supuesto que la propia seccion destaca como el que dejaria «el ciclo entero del producto
...
- **Por que importa:** el dato de fondo es correcto —con la ruta real da el `0` publicado—, pero la
  orden no se puede pegar y correr, que es lo unico que `CLAUDE.md` le pide a una evidencia.
  `decisions.md` no es un archivo agnostico, asi que nada obligaba a esconder la ruta: las demas
  ordenes de la misma sesion la escriben literal. Es **Baja** porque la conclusion de `D-153` no
  depende de ella y la ruta esta declarada en `project.md`.
- **Por que es hallazgo y no solo una nota del informe:** el cierre lo dejo escrito en su seccion 6,
  pero `decisions.md` no es del cierre y no quedo ninguna `T-XXX` — el mismo patron que `F-103`.
...
- **Por que importa:** no es un cambio posterior al commit — `T-167` edito ese archivo dentro de la
  misma sesion, despues de que la nota se escribiera, asi que el propio commit ya la contradecia.
  `A-018` es el supuesto que sostiene que toda diferencia con el esqueleto es una promocion
  pendiente, y esa nota es su unico ejercicio publicado. El pendiente quedo en la seccion 6 del
  informe y en una nota de `tasks.md` bajo `T-173`, no en el archivo donde se lee la cifra.
- 🔑 **Por que es Baja:** a diferencia de `F-103`, la nota si declara en su propia linea que mide un
  estado vivo y que «la cifra de hoy vale para hoy». Lo que falta es la constancia del desfase donde
  vive la cifra.
...
```
  ...
  Es una regresion, no la costumbre: las dos sesiones anteriores si rellenaron los dos campos.
...
- **Por que importa:** el mismo archivo afirma dos estados distintos del mismo hallazgo, y el campo
  que enlaza el hallazgo con su tarea queda vacio — el puntero a `T-172`/`T-173` solo sobrevive en la
  prosa del bloque `🕐 Tratamiento`. El control mecanico de indices solo cubre `_persistence/`, asi
  que nada lo habria cazado. Es **Media**: el registro se contradice consigo mismo, aunque el
  tratamiento de fondo este escrito y sea correcto.
- ⚠️ **Los dos campos de `F-102` y `F-103` quedan puestos por esta misma auditoria al cerrarlos**, asi
  que lo que queda por decidir es solo la parte hacia adelante.
...
🕐 **Tratamiento del 2026-09-14 (`S-041`) — ACEPTADO.** Verificado contra `HEAD` (`a3bb32e`): la vineta
sigue diciendo «tres formas del mismo defecto en `T-180`».

```
$ git show a3bb32e:_audit/S-039.md | grep -c 'tres formas del mismo defecto en `T-180`'
1
```

- **La correccion** la implementa **`T-183`**: nota fechada al final de la seccion 1 de `S-039`, con la
  orden anclada a `62ff0d2` y la atribucion correcta (dos en `T-180`, una en `T-179`), sin reescribir la
  vineta.
...
🕐 **Tratamiento del 2026-09-14 (`S-041`) — ACEPTADO.** Verificado contra `HEAD` (`a3bb32e`): la orden de
procedencia sigue dando 16 lineas y la nota que explica la 17.ª sigue en el informe.

```
$ git diff -U0 db0e613 4962db2 -- _persistence _audit ':(exclude)_audit/S-040.md' | awk '/^\+\+\+ /{f=$2} /^\+\$ /{print f" :: "$0}' | grep -vE 'git (show|grep|log|diff) [0-9a-f]{7,40}' | wc -l
16

$ git show a3bb32e:_audit/S-040.md | grep -c 'devuelve 17 lineas'
2
```

- **La correccion** la implementa **`T-184`**: nota fechada al final de la seccion 7 de `S-040`, sin
  reescribir los bloques, con la procedencia anclada a `4962db2`, el control corrido sobre las dos
  versiones commiteadas del informe (`4962db2` y `97ccbe1`) y la lectura de cada linea de la anclada,
  incluidas las 418-419 que no se habian mirado.
- **Sin regla nueva:** pegar la salida de la ejecucion ya es regla (`D-157`).
...
🕐 **Tratamiento del 2026-09-14 (`S-041`) — ACEPTADO.** Verificado contra `HEAD` (`a3bb32e`): los dos
resultados siguen sin orden.

```
$ git show a3bb32e:_persistence/tasks.md | sed -n '/el borrador de `_outbound\/` se pone al dia/,/^### T-163/p' | grep -c '^```'
0

$ git show a3bb32e:_persistence/decisions.md | grep -n "Final de linea (Paso 4)"
10271:- **Final de linea (Paso 4):** todas las parejas LF en los dos lados; copia byte a byte desde el
```

- **La correccion** la implementa **`T-185`**: nota fechada bajo `T-162` con los dos barridos —rehechos,
  porque el patron original no se publico, y la nota lo dice— anclados a `4962db2`; y nota fechada bajo
  `D-159` con la medicion del Paso 4 anclada a `db0e613` y `1748f0a`.
...
🕐 **Tratamiento del 2026-09-14 (`S-041`) — ACEPTADO, y promovido antes de corregirse.** Verificado
contra `HEAD` (`a3bb32e`):

```
$ git show a3bb32e:.claude/skills/protocol-promote/SKILL.md | grep -nE '^[0-9]+\. ' | sed -n '4,5p'
308:4. commit en el esqueleto, con un mensaje que diga **de que proyecto viene** la promocion, **que
310:4. push.
```

- **La correccion** la implementa **`T-186`**: el push pasa a «5.».
- ⚠️ **La recomendacion decia «antes de promover», y no se siguio en ese orden.** El usuario decidio
  promover la version auditada con el defecto dentro y corregirlo despues (`D-163`): corregir y promover
  en la misma sesion subiria una version sin cierre ni auditoria. El esqueleto lleva el «4.» duplicado
